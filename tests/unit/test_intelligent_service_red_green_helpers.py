...
def _structure(**kw) -> ProjectStructure:
    defaults = {
        ...
        "directories": {"app": ["app/"], "other": ["misc/"]}, "naming_style": "snake_case",
...
def _op(file_path, operation="create", description="d", instructions="", **kw) -> FileOperation:
    return FileOperation(file_path=file_path, operation=operation, description=description,
...
def test_project_context_summary_framework_fallback_and_empty():
    ...
    out = svc._build_project_context_summary(
        _structure(frameworks=[], framework="Django", directories={}, example_files={},
                   common_imports=[], project_types=[])
    )
    assert "- **Framework**: Django" in out
    out2 = svc._build_project_context_summary(
        _structure(frameworks=[], framework=None, directories={}, example_files={},
                   common_imports=[], project_types=[], naming_style="")
    )
...
def test_enhanced_context_full_file_mode():
    ...
    out = svc._build_enhanced_context(
        "add login", _analysis(), _structure(), "app.py",
        output_mode=OutputMode.FULL_FILE, operation="create",
    )
...
def test_enhanced_context_unified_diff_mode():
    svc = _make_service()
    out = svc._build_enhanced_context(
        "fix", _analysis(feature_name=None), _structure(frameworks=[], framework="Django"),
        "app.py", output_mode=OutputMode.UNIFIED_DIFF, operation="modify",
    )
...
def test_enhanced_context_other_mode():
    svc = _make_service()
    out = svc._build_enhanced_context(
        "r", _analysis(), _structure(example_files={}), "app.py",
...
def test_operation_request_with_instructions():
    ...
    out = svc._build_operation_request(
        _op("a.py", instructions="DO THIS"), _structure(), "goal"
    )
...
def test_operation_request_create_css_editor_and_plain():
    svc = _make_service()
    out = svc._build_operation_request(
        _op("style.css", description="add line number column"), _structure(), "goal"
    )
    assert "Monospace font for code" in out
    out2 = svc._build_operation_request(
        _op("theme.css", description="theme"), _structure(), "goal"
    )
...
def test_operation_request_create_js_editor_and_plain():
    svc = _make_service()
    out = svc._build_operation_request(
        _op("editor.js", description="line number sync"), _structure(), "goal"
    )
    assert "generate line numbers" in out
    out2 = svc._build_operation_request(
        _op("util.js", description="utils"), _structure(), "goal"
    )
...
def test_operation_request_create_html_editor_and_plain():
    svc = _make_service()
    out = svc._build_operation_request(
        _op("templates/editor.html", description="editor page"), _structure(), "goal"
    )
    assert "contenteditable" in out
    out2 = svc._build_operation_request(
        _op("index.html", description="home"), _structure(), "goal"
    )
...
def test_operation_request_create_py_service_test_plain():
    svc = _make_service()
    out = svc._build_operation_request(
        _op("svc.py", description="add service endpoint"), _structure(), "goal"
    )
    assert "Route/endpoint definitions" in out
    out2 = svc._build_operation_request(
        _op("test_editor.py", description="tests"), _structure(), "goal"
    )
    assert "Test cases for line number functionality" in out2
    out3 = svc._build_operation_request(
        _op("mod.py", description="module"), _structure(), "goal"
    )
...
def test_operation_request_create_other_ext():
    svc = _make_service()
    out = svc._build_operation_request(
        _op("data.json", description="data"), _structure(), "goal"
    )
...
def test_operation_request_main_py_enhancement_no_duplication():
    svc = _make_service()
    out = svc._build_operation_request(
        _op("main.py", operation="modify", description="add route for editor",
            instructions="has **Important for main.py modifications** already"), _structure(), "goal"
    )
...
def test_operation_request_dependencies_template_and_conventions():
    svc = _make_service()
    out = svc._build_operation_request(
        _op("b.py", dependencies=["a.py"], template_file="tpl.py"), _structure(), "goal"
    )
...
def test_operation_request_framework_fallback():
    svc = _make_service()
    out = svc._build_operation_request(
        _op("b.py"), _structure(frameworks=[], framework="Django"), "goal"
    )
    assert "**Framework**: Django" in out
...
def test_generate_multi_file_explanation_with_failures():
    ...
    out = svc._generate_multi_file_explanation(plan, [
...
def test_analyze_failure_patterns_empty_patch_diff_recommendation():
    ...
    svc._analyze_failure_patterns({
        "success": False, "error": "empty_patch: none", "output_mode_used": "unified_diff",
        "target_file": "app.py",
    })
...
def test_analyze_failure_patterns_git_apply_and_unknown():
    svc = _make_service()
    svc._analyze_failure_patterns({
        "success": False, "error": "git_apply_check_failed: ctx", "output_mode": "full_file",
        "target_file": "noext",
    })
    svc._analyze_failure_patterns({
        "success": False, "error": "something else", "target_file": "unknown",
    })
...
def test_plan_to_dict():
    svc = _make_service()
    plan = ExecutionPlan(
        original_request="r", complexity="complex", strategy="parallel",
        success_criteria=["ok"],
