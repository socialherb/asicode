...
class FakeGit:
    ...
    def pre_commit_calls(self) -> int:
        return len([c for c in self.calls if any("pre_commit" in a or "pre-commit" in a for a in c[0])])
...
def test_installed_hook_shell_quotes_paths_and_pins_interpreter(repo, monkeypatch):
    ...
    (weird / "scripts" / "git_commit_wrapper.py").write_text("print('WRAPPER_REACHED')\n", encoding="utf-8")
