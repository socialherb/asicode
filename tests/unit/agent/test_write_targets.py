...
APPLY_PATCH_SHAPES = {
    "git-prefix": {"patch": "diff --git a/app.py b/app.py\n--- a/app.py\n+++ b/app.py\n" + _PATCH_BODY},
...
@pytest.mark.parametrize("tool_name,shape,args", ALL_SHAPES, ids=[f"{t}-{s}" for t, s, _ in ALL_SHAPES])
...
def test_patch_parser_ignores_dev_null():
    targets = parse_patch_targets("--- /dev/null\n+++ b/new.py\n@@ -0,0 +1 @@\n+x = 1\n")
...
def test_no_approval_prompt_for_the_nonexistent_delete_file_tool(repo):
    ...
    prompts: list = []
    config = AgentConfig()
    config.approval_callback = lambda tool, args, preview: prompts.append((tool, preview)) or True
...
def test_malformed_ops_reach_the_handler_error_not_a_traceback(repo):
    ...
    """
    reg = ToolRegistry(repo_root=str(repo), config=AgentConfig())
    result = reg.dispatch("write_plan", {"plan": {"kind": "ASICODE_PLAN_V1", "ops": ["oops"]}})
    assert result.ok is False
