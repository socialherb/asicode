...
class TestGetModeMatcher:
    def test_builds_once_and_caches(self, monkeypatch):
        monkeypatch.setattr(
            "external_llm.agent.semantic_intent.SemanticIntentMatcher", _StubMatcherClass)
...
class TestKeywordAnalysis:
    ...
    def test_semantic_backstop_with_digit(self, monkeypatch):
        monkeypatch.setattr(emc, "_get_mode_matcher",
                            lambda: SimpleNamespace(matches=lambda t, label: True))
    ...
    def test_semantic_backstop_no_match_falls_to_normal(self, monkeypatch):
        monkeypatch.setattr(emc, "_get_mode_matcher",
                            lambda: SimpleNamespace(matches=lambda t, label: False))
...
class TestAnalyzeRequest:
    def test_llm_decision_wins(self, monkeypatch):
        monkeypatch.setattr(emc, "_analyze_intent_with_llm_if_available",
                            lambda p, t: ExecuteMode.INTELLIGENT)
...
class TestLLMPath:
    ...
    def test_service_unavailable_returns_none(self, monkeypatch):
        monkeypatch.setattr(
            "external_llm.intelligent_service.create_intelligent_service_from_env",
            lambda a, b: None)
    ...
    def test_service_creation_raises_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "external_llm.intelligent_service.create_intelligent_service_from_env", boom)
        assert emc._analyze_intent_with_llm_if_available("x", None) is None
    ...
    def test_alias_exact_match(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "external_llm.intelligent_service.create_intelligent_service_from_env",
            lambda a, b: _fake_service(client))
    ...
    def test_fuzzy_alias_match_inside_prose(self, monkeypatch):
        client = _FakeClient(response=SimpleNamespace(
            content="I recommend the intelligent mode for this."))
        monkeypatch.setattr(
            "external_llm.intelligent_service.create_intelligent_service_from_env",
            lambda a, b: _fake_service(client))
