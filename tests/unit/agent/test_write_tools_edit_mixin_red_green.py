...
def test_edit_file_read_failure(tool_registry):
    ...
    p.chmod(0)
    try:
        res = tool_registry.dispatch("edit_file", {"path": "locked.txt", "operations": [{"type": "replace", "anchor": "x", "content": "y"}]})
        assert not res.ok
...
def test_edit_file_content_anchor_ratio_warning(tool_registry):
    ...
    res = tool_registry.dispatch(
        "edit_file",
        {"path": "w.py", "operations": [{"type": "replace", "anchor": "x = 1", "content": "x = 1\n" + ("y = 2\n" * 100)}]},
    )
...
def test_edit_file_write_failure(tool_registry, monkeypatch):
    ...
    res = tool_registry.dispatch("edit_file", {"path": "wf.txt", "operations": [{"type": "replace", "anchor": "x", "content": "y"}]})
    assert not res.ok
...
def test_edit_file_replace_op_type_mismatch_warning(tool_registry):
    ...
    assert res.ok, res.error
    assert any("op-type mismatch" in w or "structurally always removes" in w for w in res.metadata.get("edit_warnings", []))
...
def test_apply_scoped_replacement_fallback_splice(tool_registry):
    ...
    out = WriteToolsEditMixin._apply_scoped_replacement(
        tool_registry, content, "f.py", "b = 2", "c = 3", (2, 2),
    )
    assert out["ok"], out
...
def test_apply_one_edit_fallback_contexts_and_replace_all(tool_registry):
    ...
    out = WriteToolsEditMixin._apply_one_edit_text(
        tool_registry, content, "f.py", "x = 1", "y = 2", False,
    )
    assert not out["ok"]
    ...
    out2 = WriteToolsEditMixin._apply_one_edit_text(
        tool_registry, content, "f.py", "x = 1", "y = 2", True,
    )
    assert out2["ok"], out2
...
def test_edit_text_batch_validation_errors(tool_registry):
    ...
    res4 = tool_registry.dispatch("edit_text", {"file_path": "bv.py", "edits": [{"old_string": "a", "new_string": "b", "scope_start_line": "z", "scope_end_line": 1}]})
...
def test_edit_text_non_python_gate_refusal_and_soft_fail(tool_registry, monkeypatch):
    ...
    try:
        res = tool_registry.dispatch(
            "edit_text", {"file_path": "gate.ts", "old_string": "let x = 1;", "new_string": "let x = ("},
        )
        assert not res.ok
...
def test_edit_text_non_python_gate_provider_exceptions(tool_registry, monkeypatch):
    ...
    res = tool_registry.dispatch(
        "edit_text", {"file_path": "gate2.ts", "old_string": "let x = 1;", "new_string": "let y = 2;"},
    )
    assert res.ok, res.error
    ...
    class _SoftProv:
        ...
        def validate_syntax(self, path, content):
            return types.SimpleNamespace(
                ok=False, errors=[_Err("s.ts", 1, 2, "undefined name 'zzz'")]
            )
...
def test_modify_symbol_dry_run_snapshot_failure(tool_registry):
    ...
    try:
        res = tool_registry.dispatch(
            "modify_symbol", {"file_path": "ms.py", "symbol": "f", "code": "def f():\n    return 1", "dry_run": True},
        )
        assert not res.ok
