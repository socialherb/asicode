...
class TestEditBlocksTolerrant:
    ...
    def test_create_file_existing_gives_clear_error(self, registry, git_repo):
        ...
        result = registry.dispatch(
            "write_plan",
            {"plan": {"kind": "ASICODE_PLAN_V1", "ops": [{"op": "create_file", "path": "main.py", "content": "x"}]}},
        )
...
class TestBeforeTextIndentEnrichment:
    ...
    def test_screenshot_case6_abs_path_stripped_unique_before_ok(self, indent_registry, indent_repo):
        ...
        result_ambiguous = indent_registry.dispatch(
            "write_plan",
            {
                "plan": {
                    "kind": "ASICODE_PLAN_V1",
        ...
        assert "matched 2" in (result_ambiguous.error or "").lower() or "matched" in (result_ambiguous.error or "")

        # With unique before (4-space indent → unique, 2-space line doesn't contain 4-space prefix)
        result_ok = indent_registry.dispatch(
            "write_plan",
            {
                "plan": {
                    "kind": "ASICODE_PLAN_V1",
...
class TestWritePlanRegressions:
    def test_valid_plan_v1_still_works(self, registry, git_repo):
        result = registry.dispatch(
            "write_plan",
            {"plan": {"kind": "ASICODE_PLAN_V1", "ops": [{"op": "create_file", "path": "reg.py", "content": ""}]}},
        )
        assert result.ok
    ...
    def test_ops_key_in_args_still_works(self, registry, git_repo):
        result = registry.dispatch("write_plan", {"ops": [{"op": "create_file", "path": "reg3.py", "content": ""}]})
        assert result.ok
...
class TestExtractTruncatedOpPath:
    ...
    def test_escaped_quote_inside_content_does_not_close_string(self):
        ...
        raw = '{"ops":[{"op":"create_file","path":"a.py","content":"he said \\"hi\\" then \\"path\\": \\"b.py\\" more'
        hint = _extract_truncated_op_path(raw)
