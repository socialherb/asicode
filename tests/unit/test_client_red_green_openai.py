...
def test_chat_streaming_path_via_token_callback(monkeypatch):
    ...
    out = client.chat(
        [LLMMessage(role="user", content="hi")], model="gpt-4",
...
def test_chat_streaming_callback_failure_tolerated(monkeypatch):
    ...
    out = client.chat(
        [LLMMessage(role="user", content="hi")], model="gpt-4", token_callback=_bad,
    )
...
def test_tools_streaming_callback_failure_tolerated(monkeypatch):
    ...
    def _bad(_c):
        raise RuntimeError("ui broke")
    out = client.chat_with_tools(
        [LLMMessage(role="user", content="hi")], tools=[], model="gpt-4", token_callback=_bad,
    )
    assert out.content == "x"
...
def test_zai_chat_thinking_disabled(monkeypatch):
    client, captured = _zai_client(monkeypatch)
    client.chat([LLMMessage(role="user", content="hi")], model="glm-5.2",
                thinking_mode=False, reasoning_effort="high")
...
def test_zai_chat_with_tools_thinking_enabled_with_effort(monkeypatch):
    ...
    client.chat_with_tools([LLMMessage(role="user", content="hi")], tools=[],
                           model="glm-5.2", thinking_mode=True, reasoning_effort="high")
    assert captured["thinking"] == {"type": "enabled"}
    assert captured["reasoning_effort"] == "high"
...
def test_zai_chat_with_tools_effort_without_thinking(monkeypatch):
    client, captured = _zai_client(monkeypatch)
    client.chat_with_tools([LLMMessage(role="user", content="hi")], tools=[],
                           model="glm-5.2", reasoning_effort="minimal")
...
def test_openrouter_chat_strips_prefix_injects_pref_logs_cache(monkeypatch):
    captured = {}
    resp = _FakeResp(json_value=_ok_choices(usage={"prompt_tokens": 100,
    ...
    out = client.chat([LLMMessage(role="user", content="hi")],
...
def test_openrouter_log_cache_ptd_without_cached_tokens():
    OpenRouterClient._log_cache({"usage": {"prompt_tokens": 10,
                                           "prompt_tokens_details": {"foo": 1}}})
