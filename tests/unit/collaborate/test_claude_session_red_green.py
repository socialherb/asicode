...
class TestStreamLoopDispatch:
    ...
    def test_all_message_types_dispatched(self):
        ...
        assistant = AssistantMessage(
            content=[TextBlock(text="analysis"), ToolUseBlock(id="t1", name="bash", input={"cmd": "ls"})],
            model="claude-sonnet",
        )
        user = UserMessage(
            content=[
        ...
        result = ResultMessage(
            ...
            structured_output={"status": "success", "summary": "done", "confidence": 0.9},
        )
    ...
    def test_fallback_uses_accumulated_text(self):
        assistant = AssistantMessage(
            content=[TextBlock(text="full analysis body")],
            model="m",
        )
        _s, result = self._collect([assistant])
...
class TestAssistantMessageBlocks:
    ...
    def test_text_block_appends_and_emits(self):
        ...
        s._handle_assistant_message(
            SimpleNamespace(content=[TextBlock(text="hello")]),
            acc,
        )
        assert acc == ["hello"]
    ...
    def test_empty_text_block_skipped(self):
        ...
        acc = []
        s._handle_assistant_message(
            SimpleNamespace(content=[TextBlock(text="")]),
            acc,
        )
        assert acc == []
        assert s._events == []
...
class TestEmitToolResult:
    ...
    def test_is_error_and_missing_id_defaults(self):
        ...
        s._emit_tool_result(
            SimpleNamespace(
                content="fail",
                is_error=True,  # tool_use_id attribute absent
            )
        )
        e = s._events[0]
