...
def _ev(
    ...
    return TriggerEvent(kind=kind, repo_root=".", source_file=source_file, metadata=metadata or {})
...
def test_feature_gate_disabled_returns_ignore():
    ...
    p = TriggerPolicy(
        model_tier="strong",
        enabled_features={"file_review"},  # only file_review on
    )
...
def test_route_test_failed_suggest():
    ...
    dec = p.evaluate(
        _ev(
            ...
            metadata={"failing_tests": ["test_x", "test_y"], "first_traceback": "tb", "summary_line": "2 failed"},
        )
    )
    assert dec.kind == ActionKind.SUGGEST
