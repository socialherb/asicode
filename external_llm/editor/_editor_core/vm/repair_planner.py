...
class RepairPlanner:
    ...
    def __init__(self, language: str, classifier: BaseFailureClassifier):
        self._classifier = classifier
        self._registry = RepairRegistry(language)
    ...
    def plan(
        ...
        # Use typed classification to get symbol and evidence source
        classification = self._classifier.classify_typed(errors, code=code, language=self._language)
        failure_type = classification.type
        logger.info(
            "Classified failure: %s (source=%s, symbol=%s, %d errors)",
            failure_type.value, classification.source.value,
            classification.symbol, len(errors),
        )
        ...
        # Run strategy on the primary error, passing classification
        ops = strategy(code, errors[0], classification)
        if ops is None:
            return None
