...
def _run_bounded_subprocess(
    ...
    executable=None, cwd=None, input=None, env=None,
):
...
class IntelligentLLMService:
    ...
    def _decide_next_action_from_apply_reason(self, reason: str) -> str:
        """
        Map apply failure reason -> next action.
          - abort: do not retry
          - replan: re-run planning/generation with failure feedback
          - regen_full_file: force feedback hint for FULL_FILE regeneration (handled via feedback)
        """
        r = (reason or "").strip().upper()

        if r in {"WORKTREE_DIRTY"}:
            return "abort"
        if r in {"PATH_INVALID"}:
            return "replan"
        if r in {"PATCH_MALFORMED", "PYCOMPILE_FAILED"}:
            return "regen_full_file"
        if r in {"CONTEXT_MISMATCH", "CONFLICT"}:
            return "regen_full_file"
        if r in {"SKIPPED_BINARY_FILE", "SKIPPED_LARGE_FILE"}:
            return "abort"
        return "replan"

    def _apply_patch_local(self, repo_path: Path, patch_text: str) -> dict[str, Any]:
        """
        Agent-mode apply helper (server-aligned prep, best-effort apply).

        What this adds vs. raw `git apply`:
        - Normalizes patch text (same as server)
        - Cleans diff via diff_apply._clean_diff (server behavior)
        - Extracts touched files for diagnostics
        - Classifies failures into standard reasons for strategy decisions
        ...
        """
        # LEGACY FALLBACK: low-level patch application. Prefer using OperationExecutor with a proper plan.

        # ---- 0) Worktree safety guard (policy default: abort on dirty) ----
        try:
            st = _run_bounded_subprocess(
            ...
            # If status fails, still attempt apply.
            logger.warning(f"git status failed before apply: {e}")

        # ---- 1) Normalize + clean diff (align with server apply prep) ----
        try:
        ...
        try:
            ...
            # If diff_apply isn't available, continue without cleaning.
            _clean_diff = None
            extract_touched_files_from_diff = None
            logger.warning(f"diff_apply helpers unavailable; skipping clean_diff: {e}")

        patch_norm = normalize_patch_text(patch_text or "")

        # Basic PATH_INVALID detection (keep it even if cleaning is unavailable)
        lowered = patch_norm.lower()
        if "diff --git a/.." in lowered or "diff --git b/.." in lowered or "\n--- /" in patch_norm or "\n+++ /" in patch_norm:
            return {
                "success": False,
                "reason": "PATH_INVALID",
                "stdout": "",
                "stderr": "unsafe path in patch",
                "touched_files": [],
            }
        ...
        if _clean_diff is not None:
            try:
                patch_clean = _clean_diff(patch_norm, str(repo_path), file_path_hint=None)
            except Exception as e:
                # Cleaning failure strongly correlates with malformed patch
                return {
                    "success": False,
                    "reason": "PATCH_MALFORMED",
                    "stdout": "",
                    "stderr": f"clean_diff_failed: {type(e).__name__}: {e}",
                    "touched_files": [],
                }
        ...
        if extract_touched_files_from_diff is not None:
            try:
                touched_files = extract_touched_files_from_diff(patch_clean) if patch_clean.strip() else []
            except Exception:
                touched_files = []
        ...
        if not patch_clean.strip():
            return {
                ...
                "reason": "PATCH_MALFORMED",
                "stdout": "",
                "stderr": "empty patch after normalization/cleaning",
                "touched_files": touched_files,
            }

        # ---- 2) Try canonical execution pipeline via OperationExecutor ----
        fallback_stage = None
        fallback_used = False

        try:
            from external_llm.agent.tool_registry import AgentConfig, ToolRegistry
            from external_llm.editor._editor_core.lane.operation_executor import OperationExecutor
            from external_llm.editor._editor_core.lane.patch_to_operation_plan import patch_to_operation_plan

            config = AgentConfig()
            tool_registry = ToolRegistry(str(repo_path), config)
            executor = OperationExecutor(tool_registry)

            # Stage 1: Plan building
            try:
                plan = patch_to_operation_plan(str(repo_path), patch_clean, file_path_hint=None)
            except Exception:
                fallback_stage = "plan_build_failed"
                raise
            ...
            try:
                result = executor.execute_plan(
                    plan,
                    context={"source": "intelligent_service_apply"}
                )
            except Exception:
                # Could be preflight, execution, or internal error
                fallback_stage = "executor_execution_failed"
                raise

            if result.success:
                return {
                    "success": True,
                    "reason": "executor_applied",
                    "stdout": "",
                    "stderr": "",
                    "touched_files": result.modified_files,
                    "fallback_used": False,
                    "execution_id": result.run_id,
                }
            else:
                # Executor ran but verification failed or other failure
                fallback_stage = "executor_verification_failed"
                # Continue to legacy fallback
                fallback_used = True
        except Exception as e:
            # Log and continue to legacy apply
            logger.debug(f"OperationExecutor fallback: {e}")
            fallback_used = True

        # At this point, we are using legacy fallback
        fallback_used = True
        if fallback_stage is None:
            fallback_stage = "unknown"

        # ---- 2) Apply using git (same mechanics as server, but inside agent) ----
        try:
            check = _run_bounded_subprocess(
                ["git", "apply", "--check", "-"],
                cwd=str(repo_path),
                input=patch_clean,
            )
            if check.returncode != 0:
                reason = self._classify_git_apply_error(check.stderr or "")
                return {
                    "success": False,
                    "reason": reason,
                    "stdout": check.stdout,
            ...
            applyp = _run_bounded_subprocess(
                ["git", "apply", "-"],
                cwd=str(repo_path),
                input=patch_clean,
            )
            if applyp.returncode != 0:
                reason = self._classify_git_apply_error(applyp.stderr or "")
                return {
                    "success": False,
                    "reason": reason,
                    "stdout": applyp.stdout,
                    "stderr": applyp.stderr,
                    "touched_files": touched_files,
                    "fallback_used": fallback_used,
                    "fallback_stage": fallback_stage,
                }

            # ---- 3) py_compile guard (align with server apply pipeline) ----
            try:
                import py_compile

                py_files = [p for p in (touched_files or []) if LanguageId.from_path(str(p)) is LanguageId.PYTHON]
                for rel in py_files:
                    abs_path = (repo_path / str(rel).lstrip("/")).resolve()
                    # Only compile if file exists (e.g., deletes/renames in patch)
                    if abs_path.exists():
                        py_compile.compile(str(abs_path), doraise=True)
            except py_compile.PyCompileError as e:
                return {
                    "success": False,
                    "reason": "PYCOMPILE_FAILED",
                    "stdout": applyp.stdout,
                    "stderr": str(e),
                    "touched_files": touched_files,
                    "fallback_used": fallback_used,
    ...
    def _classify_git_apply_error(self, stderr: str) -> str:
        s = (stderr or "").lower()
        if "corrupt patch" in s or "patch format" in s or "unrecognized input" in s:
            return "PATCH_MALFORMED"
        if "does not apply" in s or "patch failed" in s:
            # Could be context mismatch or conflict; keep as context mismatch for strategy.
            return "CONTEXT_MISMATCH"
        if "already exists" in s or "file exists" in s:
            return "CONFLICT"
        return "UNKNOWN"

    def _extract_files_from_test_failure(self, test_res: dict[str, Any]) -> list[str]:
        """
        Best-effort extraction of relevant files from pytest failure output.

        Returns repo-relative paths (no leading slash).
        Priority:
          1) failing_tests nodeids (e.g., "tests/test_x.py::test_y")
          2) traceback patterns: File "path", line N
        """
        import re

        files: list[str] = []

        failing = test_res.get("failing_tests") or []
        if isinstance(failing, list):
            for item in failing[:30]:
                s = str(item)
                if "::" in s:
                    s = s.split("::", 1)[0]
                s = s.strip().lstrip("/")
                if LanguageId.from_path(s) is LanguageId.PYTHON and "/" in s:
                    files.append(s)

        tb = test_res.get("traceback") or ""
        if tb:
            for m in re.finditer(r'File\s+"([^"]+)"\s*,\s*line\s+\d+', str(tb)):
                p = (m.group(1) or "").strip()
                if not p:
                    continue
                p = p.replace("\\", "/")
                if LanguageId.from_path(p) is not LanguageId.PYTHON:
                    continue
                # Keep repo-ish suffixes if present; otherwise just normalize.
                p = p.lstrip("/")
                files.append(p)
    ...
    def _run_pytest_local(self, repo_path: Path) -> dict[str, Any]:
        """Best-effort pytest runner without external dependencies."""
        try:
            ...
            return {
                ...
                "summary_line": "pytest command not found",
                "stdout": "",
                "stderr": "",
            }
        except Exception as e:
        ...
        summary_line, failing_tests, traceback = self._summarize_pytest_output(out)
    ...
    def _summarize_pytest_output(self, text: str) -> tuple[str, list[str], str]:
        """Extract a best-effort summary for retry feedback."""
        lines = (text or "").splitlines()
        summary = ""
        failing: list[str] = []
        tb_lines: list[str] = []

        # Summary line heuristics: look for 'failed' summary.
        for ln in reversed(lines[-80:]):
            _item_ = ln.strip()
            if not _item_:
                continue
            if "failed" in _item_ and ("passed" in _item_ or "error" in _item_ or "warnings" in _item_):
                summary = _item_
                break
            if _item_.startswith("ERROR") or _item_.startswith("FAILED"):
                summary = _item_
                break

        # Failing tests: lines that begin with "FAILED " in -q output
        for ln in lines:
            if ln.startswith("FAILED "):
                failing.append(ln.strip())

        # Traceback: take a slice around first "E   " block or "Traceback"
        started = False
        for ln in lines:
            if "Traceback" in ln:
                started = True
        ...
        if not summary:
            summary = "pytest_failed" if ("FAILED" in text or "ERROR" in text) else "pytest_ok"

        return summary, failing[:20], "\n".join(tb_lines).strip()
