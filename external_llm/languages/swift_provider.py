"""Swift syntax provider — regex-based symbol detection.

Symbol search only; syntax validation is non-blocking (returns ok=True)
since no bundled Swift toolchain is assumed. Symbols are served via the
...
class SwiftSyntaxProvider(SyntaxProvider):
    """Swift language support (regex symbol detection)."""
    ...
    def _validate_syntax_impl(self, file_path: str, content: str) -> SyntaxValidationResult:
        # No bundled Swift toolchain — fall back to the tree-sitter syntax check,
        # which catches structural errors (this is a hard gate on write paths).
        return tree_sitter_syntax_fallback(content, LanguageId.SWIFT)
    ...
    def get_symbol_patterns(self, kind: str = "any") -> list[SymbolPattern]:
        ...
        if kind in ("function", "method", "any"):
            # func name(  or  func name<T>(  (generic parameter clause).
            patterns.append(SymbolPattern(
                kind="function",
                regex=r"\bfunc\s+{name}\s*[<(]",
                description="Swift function declaration",
            ))
        if kind in ("class", "any"):
            patterns.append(SymbolPattern(
                kind="class",
                regex=r"\bclass\s+{name}\s*[:(<{]",
                description="Swift class declaration",
            ))
            patterns.append(SymbolPattern(
                kind="struct",
                regex=r"\bstruct\s+{name}\s*[:(<{]",
                description="Swift struct declaration",
            ))
            patterns.append(SymbolPattern(
                kind="protocol",
                regex=r"\bprotocol\s+{name}\s*[:(<{]",
                description="Swift protocol declaration",
            ))
            patterns.append(SymbolPattern(
                kind="enum",
                regex=r"\benum\s+{name}\s*[:(<{]",
                description="Swift enum declaration",
            ))
        return patterns
